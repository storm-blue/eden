...
# 修复Maven Wrapper问题的脚本
set -e

echo "🔧 修复Maven Wrapper问题..."
...
# 方案1: 重新生成Maven Wrapper
echo "📦 重新生成Maven Wrapper..."
...
if command -v mvn &> /dev/null; then
    ...
    echo "✅ Maven Wrapper重新生成完成"
else
    echo "⚠️ 系统未安装Maven，使用手动方式..."
    
    # 方案2: 手动创建Maven Wrapper文件
    mkdir -p .mvn/wrapper
    
    # 下载maven-wrapper.jar
    echo "📥 下载maven-wrapper.jar..."
    curl -o .mvn/wrapper/maven-wrapper.jar https://repo1.maven.org/maven2/org/apache/maven/wrapper/maven-wrapper/3.2.0/maven-wrapper-3.2.0.jar
    
    # 创建maven-wrapper.properties
    echo "📝 创建maven-wrapper.properties..."
...
if [ -f "mvnw" ]; then
    ...
    echo "❌ Maven Wrapper创建失败"
    exit 1
fi

echo "🎉 Maven Wrapper修复完成！"
